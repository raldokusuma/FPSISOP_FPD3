...
int main(int argc, char **argv)
{
  ...
  18:	7f 31                	jg     4b <main+0x4b>
  1a:	eb 73                	jmp    8f <main+0x8f>
  1c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
        if ((fr = open(argv[i], O_RDONLY)) > 0) {
            printf(1,"File %s Sudah Ada\n",argv[i]);
  20:	8b 04 9e             	mov    (%esi,%ebx,4),%eax
int main(int argc, char **argv)
{
	int fl;
	int fr;
  ...
  23:	83 c3 01             	add    $0x1,%ebx
        if ((fr = open(argv[i], O_RDONLY)) > 0) {
            printf(1,"File %s Sudah Ada\n",argv[i]);
  26:	c7 44 24 04 76 07 00 	movl   $0x776,0x4(%esp)
  2d:	00 
  ...
  41:	e8 a4 02 00 00       	call   2ea <close>
int main(int argc, char **argv)
{
	int fl;
	int fr;
	int i;
	for (i=1;i<argc;i++){
  46:	3b 5d 08             	cmp    0x8(%ebp),%ebx
  49:	74 44                	je     8f <main+0x8f>
        if ((fr = open(argv[i], O_RDONLY)) > 0) {
  4b:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
  52:	00 
  53:	8b 04 9e             	mov    (%esi,%ebx,4),%eax
  56:	89 04 24             	mov    %eax,(%esp)
  59:	e8 a4 02 00 00       	call   302 <open>
  5e:	85 c0                	test   %eax,%eax
  60:	89 c7                	mov    %eax,%edi
  62:	7f bc                	jg     20 <main+0x20>
            printf(1,"File %s Sudah Ada\n",argv[i]);
        } else {
            fl = open(argv[i], O_CREATE);
  64:	c7 44 24 04 00 02 00 	movl   $0x200,0x4(%esp)
  6b:	00 
  6c:	8b 04 9e             	mov    (%esi,%ebx,4),%eax
int main(int argc, char **argv)
{
	int fl;
	int fr;
  ...
  75:	e8 88 02 00 00       	call   302 <open>
            close(fl);
  7a:	89 04 24             	mov    %eax,(%esp)
  7d:	e8 68 02 00 00       	call   2ea <close>
        }
        close(fr);
  82:	89 3c 24             	mov    %edi,(%esp)
  85:	e8 60 02 00 00       	call   2ea <close>
int main(int argc, char **argv)
{
	int fl;
	int fr;
	int i;
	for (i=1;i<argc;i++){
  8a:	3b 5d 08             	cmp    0x8(%ebp),%ebx
  8d:	75 bc                	jne    4b <main+0x4b>
            close(fl);
        }
        close(fr);
	}
	...
	exit();
  8f:	e8 2e 02 00 00       	call   2c2 <exit>
  94:	66 90                	xchg   %ax,%ax
  96:	66 90                	xchg   %ax,%ax
  98:	66 90                	xchg   %ax,%ax
  9a:	66 90                	xchg   %ax,%ax
...
putc(int fd, char c)
{
 ...
 53f:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 541:	b8 89 07 00 00       	mov    $0x789,%eax
...
morecore(uint nu)
{
 ...
 6fa:	e8 4b fc ff ff       	call   34a <sbrk>
  if(p == (char*)-1)
